...
class config:
    ...
    test_batch_size   = 400     # batch size for inference
    nThreads         = 1       # number of threads for data loader to use
    device           = torch.device('cuda' if torch.cuda.is_available() else 'cpu')

    #####################################################################
    # Timing mode parameters
    timing_duration = 300  # Duration of each timing run in seconds (300s = 5 min)
    timing_runs = 6  # Total number of timing runs including warmup
    timing_warmup = 1  # Number of warmup runs to discard
    timing_seed = 42  # Random seed for timing subset sampling
    timing_samples_per_obs = 400  # Number of examples to sample per observation size for timing (defaults to test_batch_size)
...
if __name__ == "__main__":
    print(f"Starting inference script...")
    inference_run()